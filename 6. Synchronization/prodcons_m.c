...
void Producer(void *dummy){
	...
	for (i = 0 ; i < NLOOPS ; i++)  {
		...
		while (Buf.counter == MAX_BUF)  {
			// NotFull condition wait and temporary unlock
...
void Consumer(void *dummy){
	...
	for (i = 0 ; i < NLOOPS ; i++)  {
		...
		// Buf empty
		while (Buf.counter == 0)  {
			// NotEmpty condition wait and temporary unlock
...
int main(){
	pthread_t tid1, tid2;
	srand(0x8888);
	// NotFull condition value
	if (pthread_cond_init(&NotFull, NULL) < 0)  {
		perror("pthread_cond_init");
		pthread_exit(NULL);
	}
	// NotEmpty condition value
